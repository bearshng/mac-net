...
parser.add_argument("--num_half_layer", type=int, dest="num_half_layer", help="Number of LISTA step unfolded", default=3)
...
parser.add_argument("--train_path", type=str, help="Path to the dir containing the training datasets.", default="data/")
...
log_file_name = "./%s/MACNet_patch_%dLayer_%dlr_%.8f.txt" % (
...
params = Params(in_channels=1, channels=args.channels,
                     num_half_layer=args.num_half_layer,rs=args.rs_real)
...
model_name = "MACNet_patch_%dLayer_%dlr_%.8f" % (args.patch_size, args.num_half_layer*2, args.lr)
